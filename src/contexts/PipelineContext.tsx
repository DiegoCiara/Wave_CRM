...
export const ModalProvider: React.FC = ({ children }) => {
  ...
  const [createDealModalState, setCreateDealModalState] =
    ...
    const [createDealContactModalState, setCreateDealContactModalState] =
      useState<boolean>(false);
  ...
  const [funnel, setFunnelState] = useState<string>("");
  ...
  const UseCreateDealContactModal = (pipelineId?: string) => {
    setSelectedPipeline(pipelineId);
    setCreateDealContactModalState(!createDealContactModalState);
  };
  ...
  const updatePipeline = async (name: string, description: string) => {
    await PipelineService.updatePipeline(updateId, name, description );
    setUpdateIdState("");
    setUpdateModalState(!updateModalState);
  };

  const createPipeline = async (name: string, description: string, funnel: string) => {
    await PipelineService.createPipeline(name, description, funnel);
    setCreateModalState(!createModalState);
  };
  ...
  const createDealContact = async (data: DealTypes) => {
    await PipelineService.createDeal(data);
    setSelectedPipeline("");
    setCreateDealContactModalState(!createDealContactModalState);
  };
  ...
  const getPipelines = async () => {
    setLoading(true);
    try {
      const pipelinesData = await PipelineService.getPiplines();
      ...
      const pipelinesWithNames = await Promise.all(
        pipes.map(async (pipeline) => {
          const pipelineWithName = { ...pipeline };
          const pipelineInfo = await PipelineService.getPipline(pipeline.id);
          pipelineWithName.name = pipelineInfo.name;
          return pipelineWithName;
        })
      );
  
      setPipelines(
        pipelinesWithNames.sort((a, b) =>
          a.createdAt < b.createdAt ? -1 : a.createdAt > b.createdAt ? 1 : 0
        )
      );
      setElements(generateDealsList(pipelinesWithNames, dealsData));
      setRemoveElementsFiltered(generateDealsList(pipelinesWithNames, dealsData));
      setLoading(false);
    } catch (err) {
      setLoading(false);
      setError(
  ...
  const filterDeals = (
    ...
    const filteredList = Object.values(list).map((pipe) => {
      const deals = pipe.deals.filter((deal) => {
        if (deal.name.toLowerCase().includes(value.toLowerCase())) {
          return true;
        } else {
          switch (typeValue) {
            case "contact":
              ...
              case "company":
                ...
                break;
                case "user":
                  if (deal.activity[0].createdBy?.id.includes(value)) {
                    return true;
                  }
                  break;
                  case "tag":
