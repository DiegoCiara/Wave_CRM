...
export const ContactProvider: React.FC = ({ children }) => {
  ...
  const [createContactModalPipe, setCreateContactModalPipe] = useState<boolean>(false);
  ...
  const useCreateContactModalPipe = () => {
    setCreateContactModalPipe(!createContactModalPipe);
  };
  ...
  const getContacts = async (): Promise<void> => {
    ...
    try {
      ...
      setLoading(false);
      setError(
        "Não foi possivel buscar contatos, verifique sua conexão e tente novamente"
      );
    }
  };
    ...
    switch (typeValue) {
      case "socialName":
        filteredContacts = contacts.filter((contact) =>
          contact.socialName.toLowerCase().includes(terms.toLowerCase())
        );
        break;
        case "name":
          filteredContacts = contacts.filter((contact) =>
