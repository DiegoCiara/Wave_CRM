...
import DealDetailCard from "ui/components/UserDetailCard/UserDetailCard";
  ...
  return (
    <DealCardContainer onClick={props.onClick}>
      ...
      >
        ...
        >
          <DealPictureStyled
            sx={{ ml: 0.5, mt: 0.5,backgroundColor: theme.palette.primary.main, }}
          >
          </DealPictureStyled>
        </Tooltip>
      </div>
      <DealDescriptionContainer>
        <DealTitleStyled>{props?.contactName}</DealTitleStyled>
        <Tooltip
          title="Contato"
          placement="top-start"
          enterDelay={500}
          leaveDelay={100}
        >
          <DealTypeStyled>{props?.title}</DealTypeStyled>
        </Tooltip>
        <DealFooterContainer>
          <Tooltip
            title="Valor"
